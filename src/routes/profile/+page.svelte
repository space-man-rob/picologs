<script lang="ts">
	...
	function getAvatarUrl(): string | null {
		...
		if (!user || !user.avatar || !user.discordId || !isValidAvatarHash(user.avatar)) return null;

		if (user.avatar.startsWith('http')) {
			return user.avatar;
		}
		// Strip extension to avoid double .png.png
		const avatarHash = user.avatar.replace(/\.(png|jpg|jpeg|gif|webp)$/i, '');
		return `https://cdn.discordapp.com/avatars/${user.discordId}/${avatarHash}.png?size=128`;
	}
